...
float sensors_deltat = 0.0f;  // integration interval for both filter schemes
...
void readSensors()
{
  ...
  Now = micros();
  sensors_deltat = ((Now - lastUpdate) / 1000000.0f); // set integration time by time elapsed since last filter update
  lastUpdate = Now;
...
void getAccelerometerResolution() {
  switch (Ascale)
  {
    case AFS_2G:
      aRes = 2.0 / 32768.0;
...
void MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz) {
  ...
  q1 += qDot1 * sensors_deltat;
  q2 += qDot2 * sensors_deltat;
  q3 += qDot3 * sensors_deltat;
  q4 += qDot4 * sensors_deltat;
...
void MahonyQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz) {
  ...
  pb = q3;
  pc = q4;
  q1 = q1 + (-q2 * gx - q3 * gy - q4 * gz) * (0.5f * sensors_deltat);
  q2 = pa + (q1 * gx + pb * gz - pc * gy) * (0.5f * sensors_deltat);
  q3 = pb + (q1 * gy - pa * gz + pc * gx) * (0.5f * sensors_deltat);
  q4 = pc + (q1 * gz + pa * gy - pb * gx) * (0.5f * sensors_deltat);
